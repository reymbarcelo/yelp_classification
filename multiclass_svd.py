...
# TODO: set using command-line flags
verbose = True
svd = True
...
if svd:
	components = 10 #int(0.5*len(X_train[0]))
	truncated_svd = TruncatedSVD(n_components=components)
	X_train = truncated_svd.fit_transform(X_train)  
	X_test = truncated_svd.transform(X_test)
...
for j in range(NUM_CLASSES):
	...
	print('{:>20s}: {:>1.8f} {:>1.8f} {:>1.8f} {:>10d}'.format(classes[j][:20], \
			precision, \
			recall, \
			f1, \
			instances))
